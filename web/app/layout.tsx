...
const themeBootstrap = `(function(){var d=document.documentElement;try{var t=localStorage.getItem('jtm-theme');d.setAttribute('data-theme',(t==='dark'||t==='light')?t:'light');var p=location.pathname,cust=/^\\/(hotspot|renew|portal|login|register)(\\/|$)/.test(p),s=localStorage.getItem('jtm-sidebar');if(cust){s='hidden';}else if(s!=='hidden'&&s!=='shown'){s=window.innerWidth<860?'hidden':'shown';}d.setAttribute('data-sidebar',s);}catch(e){d.setAttribute('data-theme','light');d.setAttribute('data-sidebar','shown');}})();`;
...
export default function RootLayout({ children }: { children: ReactNode }) {
  return (
    <html lang="en" data-theme="light" suppressHydrationWarning>
      ...
      <body>
        <Sidebar />
        <div className="app-main"><AuthGuard>{children}</AuthGuard></div>
      </body>
    </html>
  );
}
