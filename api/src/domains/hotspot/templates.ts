...
function portal(): string {
  ...
  return `https://${webHost}/hotspot`;
}

const LOGIN = (): string => `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>HUB Networks Wi-Fi</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>body{font-family:system-ui,sans-serif;background:#0a0f1e;color:#e0f0ff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}</style>
</head><body>
<p>Connecting to HUB Networks…</p>
<script>
...
location.href='${portal()}?'+p.toString();
</script></body></html>`;

const ALOGIN = (): string => `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Connected</title>
<meta http-equiv="refresh" content="2;url=$(link-orig)">
<style>body{font-family:system-ui,sans-serif;background:#0a0f1e;color:#34d399;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;text-align:center}</style>
</head><body>
...
const STATUS = (): string => `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Session — HUB Networks</title></head>
<body>
<script>
var p=new URLSearchParams({
  'mode':'status',
...
const LOGOUT = (): string => `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Logged out</title></head>
<body>
<script>
location.href='${portal()}?mode=logout&mac=$(mac)';
</script></body></html>`;

const ERROR_PAGE = (): string => `<!DOCTYPE html>
...
const RLOGIN = (): string => LOGIN().replace(`'username':'$(username)',`, `'username':'$(username)','mode':'rlogin',`);
...
export function getTemplate(name: string): { body: string; contentType: string } | null {
  const tpl = TEMPLATES[name];
  if (!tpl) return null;
  return { body: tpl.body(), contentType: tpl.contentType };
}
