...
function renderUnifiedConfig(
  ...
  lines.push(
    ...
    `/ip proxy set enabled=yes port=8181 cache-on-disk=no max-cache-size=none`,
    `/ip proxy access remove [find comment~"jtm-expired"]`,
    `/ip proxy access add src-address-list=jtm-expired action=deny redirect-to=https://${webHost}/renew comment="jtm-expired captive"`,
