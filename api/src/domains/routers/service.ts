...
function renderUnifiedConfig(
  ...
  const lines: string[] = [
    ...
    `/ip pool remove [find name=jtm-ppp-active-pool]`,
    `/ip pool remove [find name=jtm-ppp-expired-pool]`,
  ...
  if (wantsHotspot) {
    ...
    lines.push(
      ...
      `# Cookie lifetime. RouterOS 7.x uses 'http-cookie-lifetime' (controls both`,
      ...
      ``,
      `# ===== Stale mac-cookie wipe =====`,
      `# Re-Configure shouldn't carry over old mac-cookies from the previous`,
      `# profile — they would let already-seen MACs skip the new captive flow`,
      `# even if active_devices says they should re-auth.`,
      `/ip hotspot cookie remove [find]`,
      ``,
      `# ===== Post-Configure verification =====`,
      `# Fail loud if /ip hotspot add silently no-op'd. Operator running the`,
      `# script over SSH sees the [FATAL] line and knows to investigate before`,
      `# the captive flow is observed broken on a customer's phone.`,
      `:if ([:len [/ip hotspot find name=jtm-hs]] = 0) do={`,
      `  :put "[FATAL] /ip hotspot add did not create jtm-hs — captive interception is NOT active. Check above for errors (likely: bridge or pool not found, or WAN port accidentally bridged)."`,
      `} else={`,
      `  :put "[OK] hotspot jtm-hs is up — dynamic rules:"`,
      `  :put [/ip firewall nat print count-only where dynamic]`,
      `  :put "[OK] mac-auth profile active: $[/ip hotspot profile get [find name=jtm-hotspot] login-by]"`,
      `}`,
      ``,
    );
  }
  ...
  lines.push(
    ...
    `# We deliberately do NOT add a reject-expired-down (dst-address-list) rule:`,
    `# it would block legitimate REPLIES coming back to the customer (DNS, HTTP),`,
    `# defeating the captive-portal flow. Outbound block + DST-NAT-to-proxy is enough.`,
    `# Hybrid: filter by jtm-expired address-list (instant suspend/restore via`,
    `# SSH push/remove on status change — no PPP disconnect needed).`,
    ...
    `# Allows go ABOVE the reject so DNS + portal HTTPS work even for expired customers.`,
    `:local rejId [/ip firewall filter find comment="jtm-fw reject-expired-up"]`,
    `/ip firewall filter add chain=forward action=accept connection-state=established,related comment="jtm-fw allow-established" place-before=$rejId`,
    `/ip firewall filter add chain=forward action=accept protocol=udp dst-port=53 comment="jtm-fw allow-dns-udp" place-before=$rejId`,
    `/ip firewall filter add chain=forward action=accept protocol=tcp dst-port=53 comment="jtm-fw allow-dns-tcp" place-before=$rejId`,
    `/ip firewall filter add chain=forward action=accept dst-address-list=jtm-portal comment="jtm-fw allow-portal" place-before=$rejId`,
    `# Suspended/expired customers: API pushes their IP into jtm-expired list.`,
    `# DNS + portal allowed above. HTTP gets redirected via DST-NAT + proxy below.`,
    `# HTTPS to non-portal dies (no SNI hijack) — by design, customer sees the renew page.`,
    `# Expired-PPPoE captive: enable web proxy on 8181 (loopback only) and`,
  ...
  if (wantsPppoe) {
    lines.push(
      `# ===== PPPoE =====`,
      `# Two pools so active vs expired customers get IPs from different subnets.`,
      `# RADIUS reply attribute Framed-Pool routes each user to the right pool.`,
      `# Active customers get 10.7.x.x (full speed). Expired get 10.8.x.x (captive).`,
      `/ip pool add name=jtm-ppp-active-pool ranges=10.7.0.10-10.7.255.250`,
      `/ip pool add name=jtm-ppp-expired-pool ranges=10.8.0.10-10.8.255.250`,
      `/ppp profile add name=jtm-ppp local-address=10.7.0.1 remote-address=jtm-ppp-active-pool dns-server=1.1.1.1,8.8.8.8 only-one=yes`,
      ...
      `/ip firewall nat add chain=srcnat action=masquerade src-address=10.7.0.0/16 comment="jtm-nat pppoe-active"`,
      `/ip firewall nat add chain=srcnat action=masquerade src-address=10.8.0.0/16 comment="jtm-nat pppoe-expired"`,
      ``,
    );
  }
...
:do { /ip hotspot profile set [find name=jtm-hotspot] mac-auth-mode=mac-as-username-and-password } on-error={ :put "[skip] mac-auth-mode not supported" }
:do { /ip hotspot profile set [find name=jtm-hotspot] login-by=mac-cookie,mac,http-pap } on-error={ :put "[skip] mac-cookie value in login-by not supported" }
:do { /ip hotspot profile set [find name=jtm-hotspot] http-cookie-lifetime=3d } on-error={ :do { /ip hotspot profile set [find name=jtm-hotspot] mac-cookie-timeout=3d } on-error={ :put "[skip] cookie lifetime property not supported — using default" } }
:put ("[profile] login-by=" . [/ip hotspot profile get [find name=jtm-hotspot] login-by])
