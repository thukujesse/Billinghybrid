...
function renderUnifiedConfig(
  ...
  lines.push(
    ...
    `/ip firewall filter add chain=forward action=reject reject-with=icmp-admin-prohibited src-address=10.8.0.0/16 comment="jtm-fw reject-expired-up"`,
    ...
    `/ip firewall nat add chain=dstnat src-address=10.8.0.0/16 protocol=tcp dst-port=80 action=redirect to-ports=8181 comment="jtm-expired captive"`,
