...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if not self._auth():
            return self._send_json(401, {'error': 'unauthorized'})
        # POST /freeradius/reload — Restart freeradius so it re-reads the `nas`
        # clients table. We tried SIGHUP (systemctl reload) but FreeRADIUS only
        # re-reads SQL clients (`read_clients = yes` in mods-available/sql) at
        # PROCESS STARTUP, not on HUP. So restart is the only way to pick up
        # rotated secrets. Cost: 2-3s of auth blackout for ALL tenants per call.
        # Long-term mitigation: configure FreeRADIUS dynamic clients (per-NAS
        # SQL lookup with cache TTL) — separate engineering effort.
